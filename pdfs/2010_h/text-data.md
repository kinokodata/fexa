...
真理値表を分析すると、X□YはXORD（排他的論理和の否定）演算に相当する。XとYが同じ値のときに1、異なるときに0となる。
...
フィードバック制御は、出力結果を入力側に戻して制御する方式。外乱の影響が出力に現れてから、その偏差を検知して修正動作を行う事後制御である。
...
双方向リストでAとKの間にGを挿入する場合、Aの次ポインタとKの前ポインタが変更される。図表から該当するポインタを特定する。
...
ハッシュ表探索では、データが均等に分散することが理想的。一様分布では各ハッシュ値の出現確率が等しくなるため、衝突（同一ハッシュ値）の確率が最も低くなる。
...
Javaアプリケーションは、Java VM上で独立して動作するスタンドアロンプログラム。アプレットはブラウザ、サーブレットはWebサーバが必要。
...
再入可能（リエントラント）は、複数のプロセスから同時に呼び出されても正しく動作する性質。静的変数を使わず、各呼び出しが独立して実行される。
...
平均CPI = 各命令のCPI × 構成比率の総和を計算し、実行時間 = 命令数 × 平均CPI ÷ クロック周波数で求める。結果は2.8ミリ秒。
...
実効アクセス時間の式を立てて、CPU XとYの処理時間が等しいという条件から、ヒット率を計算すると0.96となる。
...
命令キャッシュは、頻繁に実行される命令を高速メモリに保持する。局所性を高めるため、頻繁に実行される処理をまとめることで効果的に利用できる。
...
平均回転待ち時間 = 60秒 ÷ 4200回転 ÷ 2 × 1000 = 約7.14ミリ秒
平均待ち時間 = 5 + 7.14 = 約12ミリ秒
...
有機ELディスプレイは、有機化合物に電圧をかけて発光させる自発光型ディスプレイで、発光ダイオード（LED）の一種。
...
3層クライアントサーバシステムは、プレゼンテーション層（クライアント）、アプリケーション層（APサーバ）、データ層（DBサーバ）で構成される。
...
よって、処理待ち時間 = ターンアラウンドタイム - CPU時間 - 入出力時間
...
各システムの稼働率を計算し、比較する。並列接続は1-(1-0.9)²=0.99、直列接続は0.9の累乗で計算。
...
1秒間の総ディスクアクセス時間 = 20件 × 4回 × 0.04秒 = 3.2秒
1台のディスクでは1秒間に1秒分の処理しかできないため、最低4台必要。
...
ハードリアルタイムシステムでは、処理の遅延が許されない。リアルタイムOSは、定められた時間制約内で確実に処理を完了する機構を提供する。
...
スプーリング（SPOOL）は、低速な入出力装置とのデータ転送を、高速な補助記憶装置を経由して行うことで、CPUの待ち時間を削減する技術。
...
仮想記憶はOSが管理するため、アプリケーションは意識する必要がない。ただし、主記憶が不足するとページング（スラッシング）が頻発し、性能が低下する。
...
UNIXでは、各ユーザーにホームディレクトリが割り当てられ、そこが個人ファイルを保存する最上位ディレクトリとなる。
...
命令を順に追跡すると：
w1 = c/d、w2 = b+w1 = b+c/d、w3 = e/f、w4 = w3-g = e/f-g、x = w2*w4 = (b+c/d)*(e/f-g)
...
6D₁₆ = 01101101₂となり、P0,P2,P3,P5,P6が1（点灯）。7セグメントLEDの配置から、数字の5が表示される。
...
フラッシュメモリは、電気的に書き込み・消去が可能な不揮発性メモリ。消去はブロック単位で行う。アはPROM、イはDRAM、エはEPROMの説明。
...
SRAMは、フリップフロップ回路を使用して1ビットを記憶する。フリップフロップは二つの安定状態（0と1）を保持できる順序回路。
...
XとYが同じ値のときに1を出力するのは、XNOR（排他的論理和の否定）回路。回路図を分析して該当するものを選ぶ。
...
アウトラインフォントは、文字の輪郭を数式で定義するため、拡大・縮小してもギザギザ（ジャギー）が発生しない。
...
テクスチャマッピングは、3Dモデルの表面に2D画像（テクスチャ）を貼り付けて、リアルな質感を表現する技術。
...
関係データベース（RDB）は、データを2次元の表（テーブル）で表現し、表間の関係は共通の列（外部キー）で関連付ける。
...
第3正規形では、推移的関数従属を排除する。商品名は商品番号に従属するため、別テーブルに分離。発注テーブルには注文番号と商品番号をキーとして注文数量を持つ。
...
元のSQL文は「在庫表にない商品番号」を抽出。NOT EXISTSを使って同じ結果を得るには、商品表から在庫表に存在しない商品番号を選択する。
...
排他制御（ロック機構）により、同時更新によるデータの不整合を防ぐ。トランザクションの一貫性を保証する重要な機能。
...
データウェアハウス（DWH）は、企業の各種データを統合的に蓄積し、分析や意思決定支援に活用するシステム。
...
送信処理、伝送時間、サーバ処理、返信の伝送時間、受信処理の合計を計算して5.0秒となる。
...
スプリッタは、電話回線の信号を音声信号とデータ信号に分離する装置。ADSLでは必須の機器。
...
NAT（Network Address Translation）は、プライベートIPアドレスとグローバルIPアドレスを相互変換し、複数の端末で1つのグローバルIPを共有できる。
...
NTP（Network Time Protocol）は、ネットワーク上のコンピュータの時刻を同期させるプロトコル。
...
RFC1918で定められたクラスCのプライベートIPアドレス範囲は192.168.0.0〜192.168.255.255。
...
CGI（Common Gateway Interface）は、Webサーバが外部プログラムを実行し、その結果を動的にWebページとして返す仕組み。
...
ディジタル署名は、メッセージの完全性（改ざんされていないこと）と送信者の認証を保証する。機密性は保証しない。
...
各文字位置でM種類の文字が選択可能で、n文字分あるため、総数は$M^n$となる。
...
公開鍵暗号方式の代表例は、RSAや楕円曲線暗号。AESは共通鍵暗号方式。
...
3ビットで表現：実行(4)、書込み(2)、読取り(1)。
3=011(書込み+読取り)、7=111(全て)から、4=100は実行のみ。
...
WAFは、Webアプリケーションレベルの攻撃（SQLインジェクション、XSS等）を検知・遮断するセキュリティ製品。
...
DFD（データフローダイアグラム）において、○はプロセス（処理）を表す。データストアは平行線、データフローは矢印で表現。
...
E-R図の関係を読み取ると、1つの発注は1つの仕入先に対して行われることが分かる。
...
オブジェクト指向では、サブクラスでスーパクラスのメソッドをオーバーライド（再定義）できる。これはポリモーフィズムの基礎。
...
温室制御システムは、センサ情報により状態が変化し、それに応じて制御を行うため、状態遷移図での設計が適切。
...
リバースエンジニアリングは、既存のプログラムコードから設計図（UML図など）を生成する作業。
...
1回で50%確定、2回で75%、3回で87.5%、4回で93.75%確定。90%以上を確定させるには4回必要。
...
クリティカルパスを特定し、作業Eが含まれる場合の短縮効果を計算。結果1日短縮できる。
...
移植性（Portability）は、ソフトウェアを異なる環境に移行させる容易さを表す品質特性。
...
Wake on LANにより、遠隔地のPCを起動してソフトウェアの更新やメンテナンスを効率的に実施できる。
...
インシデントは、ITサービスの中断や品質低下を引き起こす事象。応答遅延は典型的なインシデント。
...
IDの総数は26×26×10×10×10=676,000個。年間増加数と累計を計算すると、2012年度に上限に達する。
...
サービス時間：19時間×30日=570時間
許容停止時間：570×0.005=2.85時間≒3時間
...
システム監査規程は、組織の重要な規程であり、最高責任者である経営者が承認する。
...
外部の監査法人から採用したシステム監査人は、社内のしがらみがなく、独立性が保たれる。
...
可用性は、必要な時にシステムが利用可能であること。サーバの二重化は可用性向上の対策。
...
内部統制では、職務の分離による相互牽制が重要。情報システム部門と利用部門の分離により、データの正確性を確保。
...
BPO（Business Process Outsourcing）は、業務プロセス全体を外部委託すること。
...
BPR（ビジネスプロセス・リエンジニアリング）は、業務プロセスを根本から見直し、劇的な改善を図る手法。
...
SaaS（Software as a Service）は、インターネット経由でソフトウェアを利用するクラウドサービス。
...
ランニングコストは運用・保守にかかる継続的な費用。保守費用が該当する。
...
要件定義では、システムの機能要件・非機能要件を明確化し、システム化範囲を決定する。
...
調達プロセス：提案評価法の決定→RFP発行→提案評価→調達先選定→調達実施
...
PPMマトリックスの縦軸は市場成長率、横軸は市場占有率（相対的市場シェア）。
...
ニッチ戦略は、特定の狭い市場セグメントに特化して競争優位を築く戦略。
...
表から、商品Aの購入者10,000人の10%=1,000人が商品Qを購入すると予想している。
...
ナレッジマネジメントは、組織内の知識（暗黙知・形式知）を共有・活用して、組織全体の競争力を向上させる経営手法。
...
アクセシビリティは、高齢者や障害者を含むすべての人が情報やサービスにアクセスできることを指す。
...
MRP（Material Requirements Planning：資材所要量計画）は、生産計画から部品の必要量と発注時期を計算するシステム。
...
RFID（Radio Frequency Identification）技術を使用したICタグ（電子タグ）は、無線で情報の読み書きが可能。
...
事業部制組織は、製品や地域などで独立した事業部を設け、各事業部に利益責任を持たせる組織形態。
...
回帰直線y = 1.1x + 10から、同じ人が昨年度よりも今年度の方が高得点を取りやすい傾向がある。
...
著作権法上、特段の取決めがない場合、著作物を創作した者（法人）に著作権が帰属する。実際に開発したC社が著作権者となる。
...
労働者派遣契約は、派遣元事業主と派遣先事業主の間で締結される。派遣元と派遣労働者の間は雇用契約。
...
ISMSの確立プロセスは、まずリスクの分析と評価を行い、その結果に基づいて管理目的及び管理策を選択し、最後に適用宣言書を作成する。