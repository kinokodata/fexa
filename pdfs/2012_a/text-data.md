...
11010000を右に2ビット算術シフトすると11110100となります（算術シフトでは符号ビットが保存される）。00010100から11110100を減算すると、00010100 + 00001100（11110100の2の補数）= 00100000となります。
...
ユークリッドの互除法の実行過程：
- 1回目：175÷77=2余り21（$x_2=21$）
- 2回目：77÷21=3余り14（$x_3=14$）
- 3回目：21÷14=1余り7（$x_4=7$）
- 4回目：14÷7=2余り0（$x_5=0$）
  手順(2)は4回実行されます。
...
2分探索は$O(\log_2 n)$、線形探索は$O(n)$、ハッシュ探索は衝突がなければ$O(1)$の計算量となります。
...
大文字26文字＋数字10文字＝36種類。$2^5=32$、$2^6=64$なので、最低6ビット必要です。
...
キュー[A,B,C,D]を[D,C,B,A]にするには、全要素をスタックに移してから戻す必要があります。deq_pushを4回実行して全要素をスタックに移し、その後pop_enqで戻すと逆順になります。
...
2分探索では探索範囲の中央値を求めるため、$(x+y)/2$でmを計算します。
...
$F(n) = n \times (n-1) \times ... \times 2 \times 1$なので、乗算は$n$回実行されます。
...
XMLは拡張可能なマークアップ言語で、ユーザが独自のタグを定義でき、データ交換に適しています。
...
割込み発生時は、まず特権モードへ移行し、現在の状態を退避し、割込み処理ルーチンのアドレスを決定してから実行します。
...
実効アクセス時間 = ヒット率×10 + (1-ヒット率)×(10+60) = 15
ヒット率をhとすると、10h + 70(1-h) = 15
10h + 70 - 70h = 15
-60h = -55
h = 0.917 ≈ 0.9
...
ディスクストライピングは、データを複数のディスクに分散配置してアクセス性能を向上させる技術です。
...
3層クライアントサーバシステムでは、プレゼンテーション層（クライアント）、アプリケーション層、データ層に分離され、業務処理はサーバ側で実行されます。
...
デュアルシステムは、2つのシステムが同じ処理を並行して実行し、結果を照合することで信頼性を高めるシステムです。
...
最初の要素の稼働率P、並列部分の稼働率は$1-(1-P)^2$が2つ直列なので、全体の稼働率は$P \times \{1-(1-P)^2\}^2$となります。
...
予防保守は故障を未然に防ぐことで、平均故障間隔（MTBF）を長くし、システムの信頼性を向上させます。
...
最短ジョブ優先（SJF）方式では、処理時間の長いタスクがスタベーション（飢餓状態）に陥る可能性があります。
...
フラグメンテーション（断片化）は、メモリ領域が細切れになる現象です。コンパクションはその解決策です。
...
LRU（Least Recently Used）方式では、最も長く使われていないページを置き換えます。ページ6アクセス時点で、最も古くアクセスされたのはページ4です。
...
動的リンキングは、プログラム実行時に必要なモジュールを結合する方式です。
...
EclipseはオープンソースのIDE（統合開発環境）です。GCCはコンパイラ、LinuxはOS、TomcatはWebコンテナです。
...
NANDゲートを2つ使った回路で、ド・モルガンの法則により、$\overline{\overline{X} \cdot \overline{Y}} = X + Y$となります。
...
フリップフロップは、0と1の2つの安定状態を持ち、1ビットの情報を記憶できる順序回路です。
...
CSS（Cascading Style Sheets）は、HTMLやXMLの表示スタイルを定義する言語です。
...
受注ファイルには担当者コードと顧客コードがあり、これらをキーとして集計可能です。商品コードは受注ファイルにないため、商品別の集計はできません。
...
E-R図では、エンティティ間の関係を1対1、1対多、多対多などのカーディナリティで表現します。
...
インデックスは、データの検索を高速化するためのデータ構造です。
...
命名規約では値の範囲までは規定できないため、同じ名前でも異なる範囲を持つ可能性があります。
...
S003の仕入値を150,000にすると、売値-仕入値=190,000-150,000=40,000となりますが、現在は190,000-120,000=70,000で条件を満たしています。更新後も条件を満たすため、他の選択肢を検討すると、イが最も適切です。
...
原子性は、トランザクションが全て実行されるか全く実行されないかのどちらかであることを保証する特性です。
...
F020という顧客番号は顧客表に存在しないため、参照整合性制約に違反します。
...
転送データ量：1,000バイト×1.2×8ビット=9,600ビット
2秒間の回線容量：64,000×2=128,000ビット
回線利用率：9,600÷128,000×100=7.5％
...
MACアドレスは48ビットで、先頭24ビットがOUI（組織固有識別子）、後続24ビットが製造番号です。
...
ネットワーク層（第3層）は、経路制御（ルーティング）とパケットの中継を行います。
...
スイッチングハブは、MACアドレステーブルを参照して、宛先ポートのみにフレームを転送します。
...
サブネットマスク255.255.255.240は/28で、16個のアドレスごとにサブネット。192.168.1.17と192.168.1.29は同じ192.168.1.16/28サブネットに属します。
...
DNSキャッシュポイズニングは、DNSサーバのキャッシュに偽の情報を注入する攻撃です。
...
公開鍵暗号方式で暗号化する場合、受信者（Bさん）の公開鍵で暗号化し、Bさんの秘密鍵で復号します。
...
署名の速度や筆圧は行動的特徴です。ア（静脈）、ウ（虹彩）、エ（指紋）は身体的特徴です。
...
SQLインジェクションは、入力フォームにSQL文を注入してデータベースを不正操作する攻撃です。
...
ハッシュ値の比較により、ファイルの改ざんを確実に検知できます。
...
8進数3=011(2進)で読取り・書込み可、実行不可から、ビットは(実行,書込み,読取り)の順。4=100(2進)なので実行のみ可能です。
...
逆アセンブルにより、バイナリコードをアセンブリ言語に変換してウイルスの動作を解析できます。
...
ペネトレーションテスト（侵入テスト）は、実際に攻撃を試みて脆弱性を発見する手法です。
...
フールプルーフは、人間の誤操作を想定した安全設計です。アはフェールセーフ、イはフェールソフト、ウはフォールトトレラントです。
...
オブジェクト図は、特定時点でのインスタンス（オブジェクト）間の関係を表現します。
...
カプセル化は、データとメソッドを一体化し、内部実装を隠蔽する概念です。
...
ブラックボックステストは内部構造を見ないため、冗長なコードは検出できません。
...
運用テストは、実際の運用環境で利用者の要求を満たすかを確認するテストです。
...
外部設計では、ユーザインタフェースである画面レイアウトなどを設計し、承認を受けます。
...
WBS（Work Breakdown Structure）は、プロジェクトの作業を階層的に細分化した構造図です。
...
クリティカルパス上の作業を短縮する必要があります。図から判断して、AとDを短縮することで全体を2日短縮できます。
...
フルバックアップの間隔が2倍になると、復旧時に適用するジャーナル量が増え、復旧時間が長くなります。
...
一斉移行は、旧システムから新システムへ一度に切り替えるため、トラブル時の影響が大きくなります。
...
問題管理は、インシデントの根本原因を分析し、再発防止を図るプロセスです。
...
機密性の観点から、アクセスコントロールの適切な設定が重要です。
...
リスクアセスメントでは、リスクの発生可能性と影響度を評価して優先順位を決定します。
...
営業秘密の要件として、秘密として管理されていることが必要で、秘密表示はその管理方法の一つです。
...
可用性は、必要な時にシステムが利用できることを指し、サーバの二重化は可用性向上の施策です。
...
BPM（Business Process Management）は、業務プロセスの継続的な改善を行う管理手法です。
...
SaaS（Software as a Service）は、インターネット経由でソフトウェアを提供するサービスです。
...
ROI = (利益の合計 - 投資額) / 投資額 × 100 で計算し、最も高い値を選択します。
...
ランニングコストは運用段階で継続的に発生する費用で、保守費用が該当します。
...
企画プロセスでは、経営上のニーズや業務上の課題などを明確にします。
...
SWOT分析の外部要因は機会（Opportunities）と脅威（Threats）で、新規参入は脅威に該当します。
...
コストプラス法は、原価に一定の利益を加算して価格を決定する方法です。
...
CRM（Customer Relationship Management）は、顧客との長期的な関係構築により、顧客生涯価値を最大化することを目的とします。
...
ナレッジマネジメントは、組織内の知識を共有・活用して組織全体の能力を向上させる経営手法です。
...
デジタルデバイドは、情報技術の利用環境の差による社会的・経済的格差を指します。
...
セル生産方式は、少人数で製品を完成させる方式で、多品種少量生産に適しています。
...
ロングテール現象は、売れ筋商品以外の多数の商品の売上合計が大きな割合を占める現象です。
...
裁量労働制は、実際の労働時間にかかわらず、あらかじめ定めた時間を労働時間とみなす制度です。
...
各仕入個数での期待値を計算し、最大となる個数を選択します。販売確率と利益・損失を考慮した期待値計算が必要です。
...
負の相関は、一方の値が増加すると他方が減少する関係で、右下がりの傾向を示します。
...
親和図法（KJ法）は、多くの情報を関連性によってグループ化し、問題を整理する手法です。
...
プログラム（コンパイラを含む）は著作物として保護されます。アルゴリズム、プログラム言語、プロトコルは保護対象外です。
...
労働者派遣では、業務の指揮命令は派遣先が直接行います。雇用関係は派遣元にあり、業務指示は派遣先が行うのが適切です。