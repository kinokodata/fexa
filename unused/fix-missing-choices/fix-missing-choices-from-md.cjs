...
// Docker環境では環境変数が直接設定されているかもしれない
if (!process.env.SUPABASE_URL && !process.env.SUPABASE_SERVICE_KEY) {
  require('dotenv').config({ path: '.env' });
}

// Dockerコンテナ内では環境変数が直接設定されている可能性があるので、
// .envファイルが読めない場合でも続行
...
function getPdfDirPath(year, season) {
  ...
  // Dockerコンテナ内での実行を想定
  return path.join('/data/pdfs', `${year}_${seasonChar}`);
}
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (questionMatch) {
      ...
      if (currentQuestion && choiceBuffer.length > 0) {
        // 重複する選択肢ラベルを除去（最初に出現したもののみ保持）
        const uniqueChoices = [];
        const seenLabels = new Set();
        
        for (const choice of choiceBuffer) {
          if (!seenLabels.has(choice.label)) {
            seenLabels.add(choice.label);
            // 正解かどうかをマーク
            choice.isCorrect = correctAnswer === choice.label;
            uniqueChoices.push(choice);
          }
        }
        
        currentQuestion.choices = uniqueChoices;
    ...
    if (currentQuestion) {
      ...
      for (const pattern of answerPatterns) {
        ...
        if (match) {
          ...
          answerMatched = true;
          console.log(`  🎯 問${currentQuestion.number} 正解検出: ${correctAnswer}`);
          break;
        }
      }
...
async function updateOrCreateChoices(questionId, mdChoices, existingChoices = []) {
  ...
  for (const mdChoice of mdChoices) {
    ...
    if (existing) {
      // 既存の選択肢を更新
      const needsTextUpdate = !existing.choice_text || existing.choice_text.trim() === '';
      const needsCorrectUpdate = existing.is_correct !== (mdChoice.isCorrect || false);
      
      if (needsTextUpdate || needsCorrectUpdate) {
        const correctIcon = mdChoice.isCorrect ? '✅' : '';
        const updateType = needsTextUpdate ? '内容更新' : '正解情報更新';
        console.log(`    ${updateType}: ${mdChoice.label}. ${mdChoice.text.substring(0, 50)}... ${correctIcon}`);
        
        const updateData = {};
        if (needsTextUpdate) updateData.choice_text = mdChoice.text;
        if (needsCorrectUpdate) updateData.is_correct = mdChoice.isCorrect || false;
        ...
        const correctIcon = mdChoice.isCorrect ? '✅' : '';
        console.log(`    スキップ: ${mdChoice.label} (既存データあり) ${correctIcon}`);
      }
    } else {
      // 新規作成
      const correctIcon = mdChoice.isCorrect ? '✅' : '';
      console.log(`    作成: ${mdChoice.label}. ${mdChoice.text.substring(0, 50)}... ${correctIcon}`);
      const { error } = await supabase
        .from('choices')
        .insert({
          ...
          is_correct: mdChoice.isCorrect || false,
          has_image: false
        });
  ...
  try {
    ...
    for (const dbQuestion of questionsToProcess) {
      ...
      if (dbQuestion.status === 'has_choices') {
        ...
        // 正解情報を更新する場合は処理を継続
        if (mdQuestion && mdQuestion.correctAnswer) {
          console.log(`  🎯 正解情報を更新: ${mdQuestion.correctAnswer}`);
          // そのまま下の処理に続く
        } else {
          // 詳細表示
          if (dbQuestion.choices && dbQuestion.choices.length > 0) {
            dbQuestion.choices.forEach(c => {
              const correctIcon = c.is_correct ? '✅' : '';
              console.log(`    ${c.choice_label}: ${c.choice_text.substring(0, 50)}... ${correctIcon}`);
            });
          }
          continue;
        }
